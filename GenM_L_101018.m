...
function [rv_A,r_cap,rv_V,hv_A,hv_V,hnr_A,hnr_V,Betav_A,Betav_V,N,Axa_A,Axa_V,Axv_A,Axv_V,Axg_A,Axg_V,AxgnrA,AxgnrV,Asg_A,Asg_V,Asnr_A,twopowerarray,Asnr_V,lv_A,lv_V,Vg_A,Vg_V,Rgnr_A,R0_C,Rgnr_V,Rg_A,Rg_V,pmmHgA,Poutnr_C,pmmHgV,Ping_A,Pout_C,Poutg_V,Sgnr_A,Sgnr_C,Sgnr_V,Sg_A,Sg_C,Sg_V]=GenM_L_101018(gamma, lambda, Pe, phbrain, Pica, Qvar)
...
gammadev = 0.5547;                            % [non-dim] Scaling factor of cross-sectional area (CSA). 
...
ica_fac  = 0.75;                              % [non-dim] Root vessel ratio.
...
l0_A     = 0.085*lvar;                        % [m]       Input arterial length. Caro&Parker. Estimated from ~half of CA (.15)
...
k_A      = kvar*1e-21;                        % [m^2]     Arterial compartmental permeability. Used in Lakin's et al., not clear how they estimated it. %N.B. Matlab's machine accuracy goes to e-7 digits. Check permeabilities.
R0_C     = (10/750)*(133*60*1e6)*R0var;%3.5467e07;      % [Pa/m^3/s]Baseline resistance of capillaries =16mmHg/ml/min taken from Linninger et al. 2009.
...
rnr_A      = (Axa_A./pi).^(1/2);
hnr_A      = (2.*rnr_A).*0.1;
Sgnr_A     = (k_A.*Asnr_A.*twopowerarray.*Ptransnr_A)./(mu_csf.*hnr_A); % s/b hv_A
...
Sgnr_V     = (k_V.*Asnr_V.*flipud(twopowerarray).*Ptransnr_V)./(mu_csf.*hnr_V); % s/b hv_V
